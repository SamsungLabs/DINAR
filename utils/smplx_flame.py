...
from smplx.lbs import lbs, vertices2landmarks, find_dynamic_lmk_idx_and_bcoords
...
class SMPL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        shapedirs = data_struct.shapedirs if not flame_merge else data_struct.shapedirs_flame
        ...
        if v_template is None:
            v_template = data_struct.v_template if not flame_merge else data_struct.v_template_flame
        if not torch.is_tensor(v_template):
            v_template = to_tensor(to_np(v_template), dtype=dtype)
        # The vertices of the template model
        self.register_buffer('v_template', v_template)

        j_regressor = data_struct.J_regressor if not flame_merge else data_struct.J_regressor_flame
        ...
        posedirs = data_struct.posedirs if not flame_merge else data_struct.posedirs_flame
...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        right_hand_pose: Optional[Tensor] = None,
        transl: Optional[Tensor] = None,
        body_scale: Optional[Tensor] = None,
        expression: Optional[Tensor] = None,
        jaw_pose: Optional[Tensor] = None,
        leye_pose: Optional[Tensor] = None,
        reye_pose: Optional[Tensor] = None,
        ...
        **kwargs
        ...
        Forward pass for the SMPLX model
            ...
            transl: torch.tensor, optional, shape Bx3
                ...
                `transl` is predicted from some external model.
                (default=None)
            body_scale: torch.tensor, optional, shape Bx3
                If given, used to scale body model
            return_verts: bool, optional
                Return the vertices. (default=True)
            return_full_pose: bool, optional
                Returns the full axis-angle pose vector (default=False)
        ...
        apply_trans = transl is not None or hasattr(self, 'transl')
        apply_scale = body_scale is not None
        if transl is None:
            if hasattr(self, 'transl'):
                transl = self.transl
        ...
        if apply_scale:
            joints *= body_scale.unsqueeze(dim=1)
            vertices *= body_scale.unsqueeze(dim=1)
