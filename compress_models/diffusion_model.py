...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.rgb_proc = torch.nn.Sequential(
            nn.BatchNorm2d(3),
            self.nonlinearity,
            torch.nn.Conv2d(3, 3, kernel_size=3, stride=1, padding=1),
        )
        self.segm_proc = torch.nn.Sequential(
            nn.BatchNorm2d(2),
            self.nonlinearity,
            torch.nn.Conv2d(2, 2, kernel_size=3, stride=1, padding=1),
        )
    ...
    def forward(self, z):
        ...
        h = self.nonlinearity(h)
        h = self.conv_out(h)
        h[:, -5:-2] = self.rgb_proc(h[:, -5:-2])
        h[:, -2:] = self.segm_proc(h[:, -2:])
        return h
